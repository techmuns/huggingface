/**
 * The weekly pipeline, with no runtime of its own.
 *
 * This file used to be a Cloudflare `WorkflowEntrypoint`, which meant the only
 * way to run the pipeline was to deploy a Worker and let Cloudflare drive it.
 ...
 * One durable step.
 *
 * Cloudflare's `WorkflowStep` satisfies this, and so does a Node shim that
 * simply calls the function. Named deliberately after the smallest thing both
 * can honestly promise: run this once, under this name, and give me back what
 * it returned. Durability across a crash is Cloudflare's addition, not part of
 * the contract — the runner recovers by re-running, which every stage is built
 * for.
 */
...
/** Exactly the eight bindings the pipeline reads, and nothing else. */
...
/**
 * What Cloudflare hands a workflow, reduced to the three fields used.
 *
 * `instanceId` becomes the run id and `timestamp` the instant every
 * "has this period finished?" question is asked about — both have to be
 * decided once and stay decided, which is why they arrive from outside rather
 * than being read from the clock mid-run.
 */
export interface PipelineEvent {
  payload?: WeeklyPipelineParams;
  timestamp: Date;
  instanceId: string;
}
...
export async function runWeeklyPipeline(
  ...
  const params = event.payload ?? {};
    ...
    const client = new HfClient(env.HF_TOKEN);
    const ingest: IngestSummary[] = [];

    for (const kind of ["model", "space"] as const) {
      ingest.push(await walk(step, env, client, kind, config.runId, config.since));
    }
  ...
  WHERE likes > 0 AND is_cluster_primary = 1;`],
    ...
    const parse = await step.do("parse", SQL_RETRY, async () => {
      ...
      const models = await parseRawModels(env.DB, config.runId);
      const spaces = await parseRawSpaces(env.DB, config.runId);
    ...
    for (let pass = 0; pass < MAX_RESOLVE_PASSES; pass++) {
      ...
      const part = await step.do(`resolve-models-${pass}`, SQL_RETRY, async () =>
        resolveModelFamilies(env.DB, RESOLVE_PAGE, from, { sweep: lastAllowedPass }),
      );
    ...
    const enrich = await enrichBlind(step, env, config.backfillWeeks);
    ...
    for (let i = config.backfillWeeks - 1; i >= 0; i--) {
      ...
      const partial = await step.do(`dedup-${target}`, SQL_RETRY, async () =>
        dedupSpaces(env.DB, `${target}T00:00:00.000Z`, targetEnd),
      );
    ...
    for (let page = 0; page < ruleCap; page++) {
      const part: ClassifyRulesSummary = await step.do(
        `classify-rules-${page}`,
        SQL_RETRY,
        async () => classifySpacesByRules(env.DB, config.since, weekEnd, rulesCursor),
      );
    ...
    const classifyLlm = await classifyWithLlm(step, env, config.since, weekEnd, config.backfillWeeks);
    ...
    for (let i = config.backfillWeeks - 1; i >= 0; i--) {
      ...
      const partial = await step.do(`aggregate-${target}`, SQL_RETRY, async () => {
        return aggregateWeeklyMetrics(env.DB, target, targetEnd);
      });
    ...
    if (!config.dryRun) {
      snapshot = await step.do("snapshot", PAGE_RETRY, async () => {
        const payload = await buildSnapshot(env.DB, config.weekStart, narrate.narrative);
        try {
          return await commitSnapshot(payload, env.GITHUB_REPO, env.GITHUB_TOKEN);
        } catch (err) {
          const reason = err instanceof Error ? err.message : String(err);
  ...
  env: PipelineEnv,
    ...
    for (let page = 0; page < pageCap; page++) {
      const result = await step.do(`ingest-${kind}-page-${page}`, PAGE_RETRY, async () =>
        ingestPage({ db: env.DB, client, kind, runId, since, cursor, page }),
      );
...
async function enrichBlind(
  ...
  backfillWeeks: number,
    ...
    await step.do("enrich-reset-errors", SQL_RETRY, async () =>
      resetTransientReadmeErrors(env.DB),
    );
...
async function classifyWithLlm(
  ...
  backfillWeeks: number,
    ...
    const modelId = env.BEDROCK_CLASSIFY_MODEL_ID;
    ...
    for (let batch = 0; batch < batchCap; batch++) {
      const result = await step.do(`classify-llm-batch-${batch}`, LLM_RETRY, async () => {
        ...
        for (let call = 0; call < CALLS_PER_STEP; call++) {
          const one = await classifySpacesByLlm(env.DB, client, modelId, weekStart, weekEnd);
          acc.total += one.total;
